...
- **BankAccount**: `id, entity_id, institution(MILLENNIUM_BCP|CGD|SANTANDER|NOVO_BANCO|REVOLUT|MANUAL|OTHER), short_code (e.g. AB, CR, REVO, MAN — the legacy “Source” code), name, iban, currency(EUR default), account_kind(CHECKING|SAVINGS|CREDIT|JOINT|MANUAL), opening_balance_eur, is_deleted, locale_format(pt_PT default)`.
...
- **CsvMapping** (NEW): `id, bank_institution, name, column_map JSONB{date_idx, amount_idx, description_idx, balance_idx, ...}, delimiter(COMMA|SEMICOLON|TAB), decimal_sep(COMMA|PERIOD), date_format(DD/MM/YYYY, ISO_8601, ...), is_deleted`. Re-usable across imports.
...
- **Rule** (NEW, auto-categorization): `id, entity_id, rule_type(KEYWORD|REGEX|MERCHANT_EXACT), pattern, category_l1_id, category_l2_id, priority, is_deleted`. Evaluated in order before merchant fallback.
