...
| `integration/test_lego_bulk_import.py` | [ADR-0048](decisions/0048-bulk-import-mirrors-the-export.md): preview resolves entity/storage/enum columns from the household's own data without ever calling Brickset; commit reuses an existing model by set number with no lookup, but does a real find-or-create lookup for a new one; a row that fails (unknown to Brickset, still carrying an error) does not sink the rest of the batch; storage import upserts on (área, contentor) instead of duplicating; and an unreadable file is refused |
