...
| Every upload fails with a 500 and **no `processing_jobs` row exists** | `PermissionError` in `documents.store_bytes`: the `storage-data` volume was written first by a root container (`./fm seed`, `./fm check`) and the API serves as uid 10001. The failure happens before the job row is created, which is why `last_error` is empty | Fixed in [ADR-0024](decisions/0024-attachment-storage-is-group-owned.md) — the tree is setgid and group-writable and the API repairs it on start. If you see it on an old volume, restart the API (`./fm up`) and it self-heals; confirm with `podman exec finmanager_api_1 ls -ld /var/lib/finmanager/storage` showing `drwxrwsr-x root finmanager` |
