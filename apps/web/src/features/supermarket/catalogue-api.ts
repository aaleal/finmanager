...
/**
 * Preview a curated product log before importing it.
 *
 * Never writes: category paths and existing-product matches are resolved so
 * the review table shows exactly what commit will do, but nothing is created
 * until the human presses "Importar".
 */
export function useBulkProductImportPreview() {
  return useMutation({
    mutationFn: (file: File) => {
      const form = new FormData();
      form.append('file', file);
      return api.upload<BulkProductImportPreview>(
        '/master-products/bulk/preview',
        form,
        undefined,
        'POST',
      );
...
/** No external lookup slows this down (unlike LEGO's Brickset call), so unlike
 * that bulk import this one is a single request, not an NDJSON stream. */
export function useBulkProductImportCommit() {
  const invalidate = useInvalidate();
  return useMutation({
    mutationFn: (rows: BulkProductImportRow[]) =>
      api.post<BulkProductImportRowResult[]>('/master-products/bulk/commit', { rows }),
...
export function useReassignItemProduct() {
  ...
  return useMutation({
    mutationFn: ({ itemId, masterProductId }: { itemId: string; masterProductId: string }) =>
      api.patch(`/supermarket-items/${itemId}/product`, { master_product_id: masterProductId }),
    onSuccess: () => {
...
export function useConfirmReceiptCategories() {
  const invalidate = useInvalidate();
  return useMutation({
    mutationFn: (receiptId: string) => api.post(`/supermarket/${receiptId}/confirm-categories`),
    onSuccess: () => {
...
export function useLegacyImport() {
  ...
  return useMutation({
    mutationFn: (file: File) => {
      const form = new FormData();
      form.append('file', file);
      return api.upload<LegacyImportResult>('/supermarket/import/legacy', form, undefined, 'POST');
    },
    onSuccess: (result) => {
      toast.success(
