...
def upload(
    db: Session, entity: Entity, owner: User, name: str, key: str | None = None
) -> SupermarketReceipt:
    receipt, _, created = supermarket_service.create_from_upload(
        db,
...
def test_a_continente_pdf_parses_and_reconciles(
    ...
    totals = supermarket_service.totals_for(db, receipt)
...
def test_a_lidl_pdf_parses_and_reconciles(
    ...
    assert supermarket_service.totals_for(db, receipt).computed_total_eur == Decimal("7.50")
...
def test_reuploading_the_same_bytes_creates_no_second_receipt(
    ...
    again, _, created = supermarket_service.create_from_upload(
        db,
    ...
    assert db.scalar(select(func.count()).select_from(SupermarketReceipt)) == 1
...
def test_two_receipts_cannot_share_an_atcud(
    ...
    clone = SupermarketReceipt(
        entity_id=entity.id, status="UPLOADED", atcud_code=original.atcud_code
    )
    db.add(clone)
    with pytest.raises(Conflict):
        supermarket_service._guard_duplicate_atcud(db, clone)
...
def test_needs_review_creates_exactly_one_task_and_confirm_resolves_it(
    ...
    assert len(tasks) == 1
    assert tasks[0].module == "supermarket"

    supermarket_service.confirm(db, receipt, actor_user_id=owner.id)
...
def test_the_status_machine_rejects_an_illegal_transition(
    ...
    supermarket_service.confirm(db, receipt, actor_user_id=owner.id)
    with pytest.raises(Conflict):
        supermarket_service.transition(db, receipt, "PARSING", actor_user_id=owner.id)

    supermarket_service.reopen(db, receipt, actor_user_id=owner.id)
    assert receipt.status == "NEEDS_REVIEW"
...
def test_a_confirmed_receipt_is_voided_with_a_reason_never_deleted(
    ...
    with pytest.raises(ValidationError):
        supermarket_service.void(db, receipt, reason="   ", actor_user_id=owner.id)

    supermarket_service.void(db, receipt, reason="devolvido na loja", actor_user_id=owner.id)
...
def test_appending_an_fs_article_to_a_confirmed_receipt_moves_no_printed_figure(
    ...
    before = supermarket_service.totals_for(db, receipt)
    total_before = receipt.total_eur

    item = supermarket_service.append_fs_item(
        db,
        receipt,
    ...
    supermarket_service.recompute(receipt)
    after = supermarket_service.totals_for(db, receipt)
...
def test_an_fs_article_without_a_value_is_refused(
    ...
    receipt = upload(db, entity, owner, "lidl-20260801.pdf")
    with pytest.raises(ValidationError):
        supermarket_service.append_fs_item(
            db,
            receipt,
...
def test_a_corrupt_file_fails_alone_and_is_retryable_from_the_stored_document(
    ...
    broken, _, _ = supermarket_service.create_from_upload(
        db,
    ...
    supermarket_service.parse_receipt(db, broken, actor_user_id=owner.id)
...
def test_the_status_board_reports_the_observed_rate_never_a_target(
    ...
    board = supermarket_service.status_counts(db, [entity.id])
