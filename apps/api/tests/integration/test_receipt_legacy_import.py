...
def test_the_2025_import_runs_end_to_end(db: Session, imported: legacy_import.ImportReport) -> None:
    ...
    flagged = db.scalars(select(Receipt).where(Receipt.status == "NEEDS_REVIEW")).all()
...
def test_fs_rows_import_with_paid_price_zero_and_disturb_no_total(
    ...
    fs_items = db.scalars(select(ReceiptItem).where(ReceiptItem.is_fs.is_(True))).all()
...
def test_every_line_resolves_to_a_product_that_carries_an_alias(
    ...
    assert all(
        ...
        for item in db.scalars(select(ReceiptItem).limit(300)).all()
    )
...
def test_rerunning_the_import_duplicates_nothing(
    ...
    receipts_before = db.scalar(select(func.count()).select_from(Receipt)) or 0
    ...
    assert (db.scalar(select(func.count()).select_from(Receipt)) or 0) == receipts_before
