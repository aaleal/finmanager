...
def test_the_ledger_link_endpoint_explains_itself(client: TestClient, receipt_id: str) -> None:
    body = client.get(f"/api/receipts/{receipt_id}/link").json()
...
def test_voiding_requires_a_reason(client: TestClient, receipt_id: str) -> None:
    assert client.post(f"/api/receipts/{receipt_id}/void", json={"reason": ""}).status_code == 422
    ok = client.post(f"/api/receipts/{receipt_id}/void", json={"reason": "devolvido"})
