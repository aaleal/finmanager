...
from app.services.supermarket import arithmetic, catalogue, extraction, fiscal, merchants, parsers
from app.services.supermarket.arithmetic import ProrationInput
from app.services.supermarket.confidence import (
    DecisionReason,
    ReceiptSignals,
    item_confidence,
    score,
...
from app.services.supermarket.parsers.base import ParsedItem, ParsedReceipt
...
def _build_items(
    ...
    rows: list[SupermarketReceiptItem] = []
...
def _item_text_quality(item: SupermarketReceiptItem) -> Decimal:
...
def _resolve_products(
    db: DbSession,
    merchant_id: uuid.UUID | None,
    items: list[SupermarketReceiptItem],
    context: list[DecisionReason],
) -> Decimal | None:
