...
from app.models.supermarket import (
    RECEIPT_TRANSITIONS,
...
from app.services.supermarket.normalize import extract_pack_weight_kg, normalize_description

MODULE = "supermarket"
ARITHMETIC_TOLERANCE = "supermarket.arithmetic_tolerance_eur"
RECEIPT_TABLE = "supermarket_receipts"
ITEM_TABLE = "supermarket_receipt_items"
...
def totals_for(db: DbSession, receipt: SupermarketReceipt) -> arithmetic.ReceiptTotals:
...
def derived(db: DbSession, receipt: SupermarketReceipt) -> dict[str, Any]:
...
def item_derived(item: SupermarketReceiptItem) -> dict[str, Any]:
...
def recompute(receipt: SupermarketReceipt) -> None:
...
def _sync_review_task(db: DbSession, receipt: SupermarketReceipt) -> None:
    """Exactly one open task per receipt in ``NEEDS_REVIEW``."""
    existing = db.scalar(
        select(ReviewTask).where(
            ReviewTask.subject_type == "SupermarketReceipt",
        ...
        if existing is None:
            db.add(
                ReviewTask(
                    entity_id=receipt.entity_id,
                    module=MODULE,
                    subject_type="SupermarketReceipt",
...
def _existing_by_hash(
    db: DbSession, entity_id: uuid.UUID, sha256_hash: str
) -> SupermarketReceipt | None:
    document = db.scalar(select(Document).where(Document.sha256_hash == sha256_hash))
    if document is None:
        return None
    return db.scalar(
        select(SupermarketReceipt).where(
            SupermarketReceipt.entity_id == entity_id,
            SupermarketReceipt.document_id == document.id,
...
def create_from_upload(
    ...
    *,
    ...
    actor_user_id: uuid.UUID | None,
    idempotency_key: str,
) -> tuple[SupermarketReceipt, ProcessingJob, bool]:
    ...
    receipt = SupermarketReceipt(entity_id=entity_id, document_id=document.id, status="UPLOADED")
    db.add(receipt)
    db.flush()
...
def _job_for(db: DbSession, receipt: SupermarketReceipt, idempotency_key: str) -> ProcessingJob:
    key = f"{idempotency_key}:{receipt.id}"
    job = db.scalar(select(ProcessingJob).where(ProcessingJob.idempotency_key == key))
    if job is None:
        job = ProcessingJob(
            idempotency_key=key,
            job_type="supermarket.parse",
            entity_id=receipt.entity_id,
            status="QUEUED",
...
def _guard_duplicate_atcud(db: DbSession, receipt: SupermarketReceipt) -> None:
    """The ATCUD is the fiscal document identity — a second one cannot exist."""
    if not receipt.atcud_code:
        return
    clash = db.scalar(
        select(SupermarketReceipt).where(
            SupermarketReceipt.entity_id == receipt.entity_id,
            SupermarketReceipt.atcud_code == receipt.atcud_code,
            SupermarketReceipt.id != receipt.id,
            SupermarketReceipt.is_deleted.is_(False),
        )
    )
...
def get_receipt(db: DbSession, receipt_id: uuid.UUID) -> SupermarketReceipt:
    receipt = db.get(SupermarketReceipt, receipt_id)
    if receipt is None or receipt.is_deleted:
        raise NotFound("Fatura não encontrada.")
    return receipt


def get_item(db: DbSession, item_id: uuid.UUID) -> SupermarketReceiptItem:
    item = db.get(SupermarketReceiptItem, item_id)
...
def update_item(
    ...
    *,
    ...
    db.flush()
    receipt = db.get(SupermarketReceipt, item.receipt_id)
...
def delete_item(
    db: DbSession, item: SupermarketReceiptItem, *, actor_user_id: uuid.UUID | None
) -> None:
...
def confirm(
    db: DbSession, receipt: SupermarketReceipt, *, actor_user_id: uuid.UUID | None
) -> SupermarketReceipt:
...
def void(
    db: DbSession, receipt: SupermarketReceipt, *, reason: str, actor_user_id: uuid.UUID | None
) -> SupermarketReceipt:
    if not reason.strip():
...
def status_counts(db: DbSession, entity_ids: list[uuid.UUID]) -> dict[str, Any]:
    ...
    live = SupermarketReceipt.is_deleted.is_(False)
    in_scope = SupermarketReceipt.entity_id.in_(entity_ids)

    total_receipts = count(select(SupermarketReceipt.id).where(in_scope, live))
    queued = count(
        select(ProcessingJob.id).where(
            ProcessingJob.job_type.like("supermarket.%"),
            ProcessingJob.status.in_(("QUEUED", "RUNNING", "RETRYING")),
        )
    )
    failed = count(
        select(ProcessingJob.id).where(
            ProcessingJob.job_type.like("supermarket.%"), ProcessingJob.status == "FAILED"
        )
    )
    to_validate = count(
        select(SupermarketReceipt.id).where(
            in_scope, live, SupermarketReceipt.status == "NEEDS_REVIEW"
        )
    )
    unresolved_lines = count(
        select(SupermarketReceiptItem.id).where(
            SupermarketReceiptItem.entity_id.in_(entity_ids),
            SupermarketReceiptItem.is_deleted.is_(False),
            SupermarketReceiptItem.master_product_id.is_(None),
        )
    )
    resolved_lines = count(
        select(SupermarketReceiptItem.id).where(
            SupermarketReceiptItem.entity_id.in_(entity_ids),
            SupermarketReceiptItem.is_deleted.is_(False),
            SupermarketReceiptItem.master_product_id.is_not(None),
        )
    )

    decided = db.execute(
        select(SupermarketReceipt.status, func.count())
        .where(
            in_scope,
            live,
            SupermarketReceipt.status.in_(("AUTO_ACCEPTED", "NEEDS_REVIEW", "CONFIRMED")),
        )
        .group_by(SupermarketReceipt.status)
    ).all()
...
def dashboard_summary(db: DbSession, entity_ids: list[uuid.UUID]) -> tuple[Decimal, int]:
    ...
    total, receipts = db.execute(
        select(
            func.coalesce(func.sum(SupermarketReceipt.total_eur), 0),
            func.count(SupermarketReceipt.id),
        ).where(
