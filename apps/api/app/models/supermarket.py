...
Slice M1a ships the ingestion spine: what the invoice claimed (``SupermarketReceipt``), what
its lines sum to (``SupermarketReceiptItem``), and how one merchant's layout is read
...
class SupermarketReceipt(Base, TimestampMixin, SoftDeleteMixin):
    ...
    *printed* — independent extracted facts. The item rows are what we *parsed*.
    ...
    __tablename__ = "supermarket_receipts"
    __table_args__ = (
        ...
        Index(
            "uq_supermarket_receipts_entity_atcud",
            "entity_id",
            "atcud_code",
            unique=True,
        ...
        Index(
            "uq_supermarket_receipts_entity_document",
            "entity_id",
            "document_id",
            unique=True,
        ...
        Index(
            "ix_supermarket_receipts_entity_purchase_date_id", "entity_id", "purchase_date", "id"
        ),
        Index("ix_supermarket_receipts_merchant_id", "merchant_id"),
        Index("ix_supermarket_receipts_status", "status"),
        CheckConstraint(
    ...
    items: Mapped[list[SupermarketReceiptItem]] = relationship(
        "SupermarketReceiptItem",
        back_populates="receipt",
        lazy="selectin",
        order_by=(
            "SupermarketReceiptItem.is_fs, SupermarketReceiptItem.line_no, "
            "SupermarketReceiptItem.created_at"
        ),
        cascade="all, delete-orphan",
    )


class SupermarketReceiptItem(Base, TimestampMixin, SoftDeleteMixin):
    ...
    __tablename__ = "supermarket_receipt_items"
    __table_args__ = (
        ...
        Index("ix_supermarket_receipt_items_receipt_id_line_no", "receipt_id", "line_no"),
        Index("ix_supermarket_receipt_items_master_product_id", "master_product_id"),
        Index("ix_supermarket_receipt_items_entity_id", "entity_id"),
    )

    id: Mapped[uuid.UUID] = uuid_pk()
    receipt_id: Mapped[uuid.UUID] = mapped_column(
        PgUUID(as_uuid=True),
        ForeignKey("supermarket_receipts.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    receipt: Mapped[SupermarketReceipt] = relationship("SupermarketReceipt", back_populates="items")
